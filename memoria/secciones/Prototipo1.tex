...
Al contar con condiciones favorables y un número normal de operaciones, la tabla permite comprobar la estabilidad operativa (\ref{tab:Escenario Base Estadísticas}):
\begin{table}[h!]
    \centering
    \renewcommand{\arraystretch}{1.3}
...
Al aumentar la carga de trabajo hasta 400 operaciones, el sistema aeroportuario comienza a operar cerca de sus límites físicos (\ref{tab:Escenario de Estrés Estadísticas}):
\begin{table}[h!]
    \centering
    \renewcommand{\arraystretch}{1.3}
    \resizebox{1.05\textwidth}{!}{
...
En este escenario, el sistema opera bajo una carga de trabajo moderadamente superior a la media y una meteorología menos favorable. Los resultados obtenidos muestran la respuesta del sistema durante dichas condiciones (\ref{tab:Escenario de Atasco Estadísticas}):
\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.3}
    \resizebox{1.05\textwidth}{!}{
...
\\
\\
Una vez identificado que el turno de tarde es el punto crítico, es interesante ver como reacciona el sistema ante la presión. Para ello, los gráficos (Figura \ref{fig:Regresion Escenario Base}, Figura \ref{fig:Regresion Escenario de Estrés} y Figura \ref{fig:Regresion Escenario Adverso}) combinan el número de operaciones aéreas con el tiempo que tardan en completar el ciclo, permitiéndonos observar la resistencia del sistema aeroportuario en cada escenario.
\begin{figure}[h!]
    \centering
