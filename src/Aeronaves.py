...
class Aeronave:
    comienzo = False
    def __init__(self,id,vueloId,estado,pasajeros,origen,destino,horaSalida,horaLlegada,horaEstacionado,horaProgramadaSalida,horaDespegue):
        self.id = id
    ...
    def infoColaAterrizaje(self):
        ...
        with open(logs,"a", newline="") as log:
            ...
            if not existeEncabezado:
                writer.writerow(["ID","ID_Vuelo","Estado","Pasajeros","Origen","Destino","Hora_Salida","Hora_Llegada","Hora_Estacionamiento","Hora_Programa","Hora_Despegue"])
            self.estado = "Llegando"
            writer.writerow([self.id,self.vueloId,self.estado,self.pasajeros,self.origen,self.destino,self.horaSalida,self.horaLlegada,self.horaEstacionado,self.horaProgramadaSalida,self.horaDespegue])
